...
단, 1회성 커스텀 슬롯 안에서 단독 아이콘이 필요한 경우에는 `DSIcon` 사용을 허용합니다.

- `DSIcon`은 컴포넌트화 대상이 아닌 1회성 커스텀 슬롯에서만 사용합니다.
- Button, Navigation Bar, Toggle, Badge 등 기존 컴포넌트 내부 아이콘에는 `DSIcon`을 직접 사용하지 않습니다. 해당 아이콘의 색상/크기/상태는 부모 컴포넌트가 소유합니다.
- 반복 사용되거나 2개 이상 화면에서 재사용될 가능성이 있는 아이콘 UI는 `DSIcon`으로 처리하지 않고 별도 컴포넌트화를 검토합니다.
- `sic.*`는 tint 적용 대상인 단색 시스템 아이콘에 사용합니다.
- `iic.*`는 브랜드/서비스 아이콘처럼 원본 컬러 유지가 필요한 아이콘에 사용합니다.
- `tint=none`은 icon 슬롯에 들어온 소스 컴포넌트의 원본 fill/binding을 그대로 따릅니다.
- `tint=neutral | brand | positive | inverse`는 `sic.*` 아이콘에 semantic color token을 직접 바인딩합니다.
