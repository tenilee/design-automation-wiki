...
단, 1회성 커스텀 슬롯 안에서 단독 아이콘이 필요한 경우에는 `Icon` 사용을 허용합니다.

- `Icon`은 컴포넌트화 대상이 아닌 1회성 커스텀 슬롯에서만 사용합니다.
- Button, NavigationBar, Toggle, Badge 등 기존 컴포넌트 내부 아이콘에는 `Icon`을 직접 사용하지 않습니다. 해당 아이콘의 색상/크기/상태는 부모 컴포넌트가 소유합니다.
- 반복 사용되거나 2개 이상 화면에서 재사용될 가능성이 있는 아이콘 UI는 `Icon`으로 처리하지 않고 별도 컴포넌트화를 검토합니다.
